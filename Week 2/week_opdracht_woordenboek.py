def main():
  ...
  if gebruiker in programmeer_termen.keys():
    ...
    print("Helaas, deze term staat niet in de woordenboek.")
  ...
  while keuze_menu > 0 or keuze_menu <= 3:
    if keuze_menu == 1:
      keuze_term = input("Welk programmeerterm wil je zoeken?")
      if keuze_term in programmeer_termen:
        print(programmeer_termen[keuze_term])
        programmeer_termen = False
      else:
        print("Zit niet in het woordenboek.")
        keuze_term = input("Welk programmeerterm wil je zoeken?")
        programmeer_termen = False

    if keuze_menu == 2:
      nieuwe_term = input("Voer hier een term in: ")
      nieuwe_term_betekenis = input("Geef de betekenis van de nieuwe term: ")
      programmeer_termen.update({nieuwe_term: nieuwe_term_betekenis})
      # break

    if keuze_menu == 3:
      print("Wat jammer! Vaarwel :)")
      # break
